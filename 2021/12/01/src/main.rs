...
impl Graph {
    ...
    fn all_paths(&self, visiting: &str, end: &str, mut path: Vec<String>) -> HashSet<String> {
        if visiting == end {
        ...
        if !visiting.chars().nth(0).unwrap().is_uppercase() && path.contains(&visiting.to_owned()) {
            return paths;
        }

        path.push(visiting.to_owned());
        for v in self.adjacency_list.get(visiting).unwrap().iter() {
            paths.extend(self.all_paths(v, end, path.clone()));
        }
...
fn main() {
    ...
    let paths = graph.all_paths("start", "end", Vec::new());
